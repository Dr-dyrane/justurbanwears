...
consent and retention evidence, provider-safety context, authority bytes, QA
evidence and lifecycle version are server-owned. Routes and browser clients
may not supply them or implement an engine port.
...
17. **Qualified native-room canvas or stop.** Final-scene stages keep the provider subject at 1024x1536 and accept only an exact approved 1024x1536 room or the versioned native 1024x1280 profile. For the 4:5 profile, every nonzero subject-alpha pixel must remain inside the guarded central window `x=16..1007`, `y=144..1391`; the app copies `x=0,y=128,w=1024,h=1280` one-to-one over the exact room. Never resize, stretch, crop, pad, extend or regenerate locked room pixels, and never silently discard visible subject pixels.
...
24. **Moderation is terminal evidence, never a retry hint.** Before dispatch, the server must resolve an exact hash-bound safety-context receipt for the operation. Garment-only stages attest that no real person is an output target; subject/final stages attest a verified adult, authorized likeness use, consented fully clothed non-sexual retail-fashion purpose. Missing, forged or stage-incompatible evidence blocks before claim or spend. A provider `moderation_blocked` response is a determinate no-output terminal result: persist its stable code, input/output/unknown stage, allowlisted coarse categories and receipt privately; create no artifact or parent; consume no semantic correction; and never auto-retry. The browser receives only a generic safety-block reason. Do not evade moderation with euphemisms, prompt churn or an unqualified provider/model/moderation-policy change.
...
The approved `juw.atelier.empty-plate.v1` is 1024x1280. It is geometrically
eligible under `juw.atelier-native-room-canvas.v1`: the 1024x1536 transparent
provider subject must fit the guarded central 1024x1280 window, which is copied
one-to-one over the exact room bytes. This clears only the former dimension
mismatch. Final-scene production remains disabled until the server runtime,
ports and canonical qualified-evaluator bundle are installed and the updated
profile is covered by closed qualification evidence.
...
from that semantic contract, its exact ordered bindings and the stage-compatible
server-owned provider-safety receipt. Routes, UI and workers may not inject a
free-form paid execution prompt or safety attestation.
...
Final-scene stages must produce a 1024x1536 transparent PNG subject, pass the
selected exact native-room profile and alpha-window gates, and be composited
one-to-one over exact room bytes by app code before
