...
const expectedChecksum = "0921c6e148cea1544ba9264614eef7bd925c7da2f9ddf826f91a96095e70c919";
...
test("the checked-in manifest validates all 23 catalogue products and immutable SKUs", () => {
  ...
  const drop02 = SHOP_CATALOGUE_MANIFEST.products.slice(-5);
  assert.deepEqual(
...
test("revision decisions no-op only for identical target evidence", () => {
  ...
  assert.equal(decideRevision({ ...request, row_count: 23 }, request), "noop");
  assert.throws(() => decideRevision({ ...request, checksum: "0".repeat(64), row_count: 23 }, request), /different checksum/);
  assert.throws(() => decideRevision({ ...request, target: "production", row_count: 23 }, request), /different target/);
  assert.throws(() => decideRevision({ ...request, row_count: 22 }, request), /row count/);
});
...
test("seed and descriptive sync never update operational inventory", () => {
  ...
  assert.equal(seed.inventory.length, 23);
