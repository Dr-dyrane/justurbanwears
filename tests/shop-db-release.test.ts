...
const expectedChecksum = "a623f0278bad2f3aaaae363272adfd14e1f420066ece4692cbb4695c504775df";
...
test("the checked-in manifest validates all 24 catalogue products and immutable SKUs", () => {
  ...
  const drop02 = SHOP_CATALOGUE_MANIFEST.products.slice(-6);
  assert.deepEqual(
...
test("revision decisions no-op only for identical target evidence", () => {
  ...
  assert.equal(decideRevision({ ...request, row_count: 24 }, request), "noop");
  assert.throws(() => decideRevision({ ...request, checksum: "0".repeat(64), row_count: 24 }, request), /different checksum/);
  assert.throws(() => decideRevision({ ...request, target: "production", row_count: 24 }, request), /different target/);
  assert.throws(() => decideRevision({ ...request, row_count: 23 }, request), /row count/);
});
...
test("seed and descriptive sync never update operational inventory", () => {
  ...
  assert.equal(seed.inventory.length, 24);
