...
const expectedChecksum = "a17908b43fca7e2e5f6eed155c1ef7e28b8fd5cc5d61f1094b3994b39dc304b7";
...
test("the checked-in manifest validates all 52 catalogue products and immutable SKUs", () => {
  ...
  const drop02 = SHOP_CATALOGUE_MANIFEST.products.slice(-34);
  assert.deepEqual(
    ...
    [
      ...
      { sku: "JUW-037", name: "Black Cropped Tee and Charcoal-Wash Wide-Leg Cargo Jeans Set", category: "Sets", price: 24500 },
      { sku: "JUW-038", name: "Black Cropped Tee and Dark Indigo Wide-Leg Cargo Jeans Set", category: "Sets", price: 24500 },
      { sku: "JUW-039", name: "Black Cropped Tee and Washed Black Wide-Leg Cargo Jeans Set", category: "Sets", price: 24500 },
      { sku: "JUW-040", name: "Plum Sparkle Cowl-Neck Jumpsuit", category: "Rompers", price: 28500 },
      { sku: "JUW-041", name: "Gunmetal Sparkle Open-Back Long-Sleeve Mini Dress", category: "Dresses", price: 28500 },
      ...
      { sku: "JUW-046", name: "Crimson Asymmetric Draped Satin Maxi Dress", category: "Dresses", price: 34500 },
      { sku: "JUW-047", name: "Scarlet Asymmetric Cascade Ruched Mini Dress", category: "Dresses", price: 29500 },
      { sku: "JUW-048", name: "Black Asymmetric Sculpted-Shoulder Mini Dress", category: "Dresses", price: 27500 },
      { sku: "JUW-049", name: "Dark Navy Draped Cowl-Neck Side-Cascade Maxi Dress", category: "Dresses", price: 32500 },
      { sku: "JUW-050", name: "Neon Chartreuse Asymmetric Ruched-Knot Maxi Dress", category: "Dresses", price: 32500 },
      { sku: "JUW-051", name: "Camel Ribbed Deep-Plunge Short-Sleeve Mini Dress", category: "Dresses", price: 27500 },
      { sku: "JUW-052", name: "Scarlet Long-Sleeve Layered-Cutout Ruched Mini Dress", category: "Dresses", price: 28500 },
      { sku: "JUW-053", name: "Red Lace-Cup Long-Sleeve Satin-Look Mini Dress", category: "Dresses", price: 28500 },
      { sku: "JUW-054", name: "Orange Halter Tie-Front Gathered Wrap Mini Dress", category: "Dresses", price: 28500 },
      { sku: "JUW-055", name: "Cobalt Long-Sleeve Crossover Cutout Mini Dress", category: "Dresses", price: 28500 },
      { sku: "JUW-056", name: "Mauve Architectural Cutout Open-Back Mini Dress", category: "Dresses", price: 28500 },
      { sku: "JUW-057", name: "Near-Black Open-Back Structured-Shoulder Midi Dress", category: "Dresses", price: 28500 },
      { sku: "JUW-058", name: "Lime One-Shoulder Rosette Ruched Mini Dress", category: "Dresses", price: 28500 },
    ],
  );
  for (const product of drop02) {
    ...
    assert.deepEqual(product.modelAnchor, product.sku === "JUW-041"
      ? { id: "lulu-v2", src: "/shop/model/lulu-v2-approved.png" }
      : { id: "lulu-v4" });
    assert.equal(product.media.filter((media) => media.slot.startsWith("MODEL_")).length, product.sku === "JUW-041" ? 0 : 3);
    assert.doesNotMatch(
...
test("revision decisions no-op only for identical target evidence", () => {
  ...
  assert.equal(decideRevision({ ...request, row_count: 52 }, request), "noop");
  assert.throws(() => decideRevision({ ...request, checksum: "0".repeat(64), row_count: 52 }, request), /different checksum/);
  assert.throws(() => decideRevision({ ...request, target: "production", row_count: 52 }, request), /different target/);
...
test("seed and descriptive sync never update operational inventory", () => {
  ...
  assert.equal(seed.inventory.length, 52);
...
test("build and deployment remain free of database administration side effects", () => {
  ...
  assert.equal(packageJson.scripts["build:vercel"], "NITRO_PRESET=vercel vite build && node scripts/fix-vercel-output-routes.mjs && node scripts/stamp-service-worker.mjs");
