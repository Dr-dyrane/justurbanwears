...
const expectedChecksum = "909a3209574cddaccafa89c29a1770698786772c996476dafb0ff3c975673d61";
...
test("the checked-in manifest validates all 15 public products and immutable SKUs", () => {
  ...
  assert.deepEqual(
    ...
    [...Array.from({ length: 14 }, (_, index) => `JUW-${String(index + 1).padStart(3, "0")}`), "JUW-016"],
  );
...
test("seed and descriptive sync never update operational inventory", () => {
  ...
  assert.equal(seed.inventory.length, 15);
