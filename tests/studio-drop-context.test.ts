...
test("keeps the canonical Drop 02 boundary at the released twenty-six pieces", () => {
...
test("keeps collection membership explicit and independent from sold or archive lifecycle labels", () => {
  ...
  assert.equal(DROP_02_COMPATIBILITY_SKUS.length, 26);
  assert.equal(new Set([...DROP_01_COMPATIBILITY_SKUS, ...DROP_02_COMPATIBILITY_SKUS]).size, 44);
