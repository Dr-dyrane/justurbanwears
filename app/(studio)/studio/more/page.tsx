...
export default function StudioMorePage() {
  return <section className="studio-more-page"><header className="studio-ops-heading"><div><p className="eyebrow">Studio</p><h1>More.</h1><p>Tools that matter, without keeping them in the way.</p></div></header><nav className="studio-more-list" aria-label="More Studio tools">{tools.map(({ href, icon: Icon, label, note }) => <Link className="studio-more-row" href={href} key={href}><Icon aria-hidden="true" size={21} strokeWidth={1.75} /><span><strong>{label}</strong><small>{note}</small></span><span aria-hidden="true">→</span></Link>)}</nav></section>;
}
