...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-20-catalogue-18` release must use checksum `a10cf80fa301af8426338502a3e57b72609762789b6df3cc30366b7e0cd53086`. It extends the reviewed manifest from 18 to 25 rows with JUW-025–JUW-031 as Drop 02: the black cropped tee and slim trouser set, violet beaded ruffle romper, black sweetheart fit-and-flare midi dress, black-and-ivory folded-neck column dress, indigo seamed denim mini dress, black cropped tee with silver ruched skirt set, and black cropped tee with pink distressed shorts set. The release uses the approved provisional prices while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-20-catalogue-17` release remains recorded by checksum `a623f0278bad2f3aaaae363272adfd14e1f420066ece4692cbb4695c504775df`.

The 25-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the seven new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
