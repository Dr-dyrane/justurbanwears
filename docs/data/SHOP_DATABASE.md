...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-22-catalogue-22` release must use checksum `23ca93e15c045f7a2821c7aa0cc50b5571eef24531ac8f628136ecd47fb9c333`. It extends the reviewed manifest from 29 to 30 rows with JUW-036 as Drop 02: the black cropped tee and blue-wash drawstring barrel trousers set. The release uses the approved provisional price while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-22-catalogue-21` release remains recorded by checksum `d55319880baf3a6e1edb62f627b449cfb63dee0a8413c1f37af03cac334cf219`.

The 30-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the twelve new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
