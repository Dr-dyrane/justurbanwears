...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-21-catalogue-20` release must use checksum `94d02da58a47009e9730a60d0f48ced24c1630c7fa383571b191d87fbf877bb6`. It extends the reviewed manifest from 18 to 27 rows with JUW-025–JUW-033 as Drop 02: the black cropped tee and slim trouser set, violet beaded ruffle romper, black sweetheart fit-and-flare midi dress, black-and-ivory folded-neck column dress, indigo seamed denim mini dress, black cropped tee with silver ruched skirt set, black cropped tee with pink distressed shorts set, black cropped tee with blue distressed shorts set, and black cropped tee with charcoal cut-off shorts set. The release uses the approved provisional prices while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-20-catalogue-19` release remains recorded by checksum `1e9d77ae8036f6311adbffa596dca073eb4864a11d9212108a9507987f5f402f`.

The 27-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the nine new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
