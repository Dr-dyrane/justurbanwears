...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-22-catalogue-21` release must use checksum `d55319880baf3a6e1edb62f627b449cfb63dee0a8413c1f37af03cac334cf219`. It extends the reviewed manifest from 27 to 29 rows with JUW-034 and JUW-035 as Drop 02: the black cropped tee and mid-thigh black cut-off shorts set, and the black cropped tee and light-wash snap-panel trousers set. The release uses the approved provisional prices while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-21-catalogue-20` release remains recorded by checksum `94d02da58a47009e9730a60d0f48ced24c1630c7fa383571b191d87fbf877bb6`.

The 29-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the eleven new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
