...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-23-catalogue-26` release must use checksum `a3bdfdab5432ce78fd887ce5dd9d9d0ed4d974a69f829c0513631a5f79437f62`. It extends the reviewed manifest from 33 to 34 rows with JUW-040 as Drop 02: the plum sparkle cowl-neck jumpsuit. The release uses the current one-piece provisional price while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-22-catalogue-25` release remains recorded by checksum `6734e12f45dd9bda1a924294d3584b07342f3776f6dd69fe6625d122197ba1a4`.

The 34-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the sixteen new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
