...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-22-catalogue-25` release must use checksum `6734e12f45dd9bda1a924294d3584b07342f3776f6dd69fe6625d122197ba1a4`. It extends the reviewed manifest from 32 to 33 rows with JUW-039 as Drop 02: the black cropped tee and washed-black wide-leg cargo jeans set. The release uses the established Drop 02 trousers-set provisional price while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-22-catalogue-24` release remains recorded by checksum `b85f941281d5d53a7c2b7127ff0115bbe03a20cb9f657d9e2edee632a6d4e43e`.

The 33-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the fifteen new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
