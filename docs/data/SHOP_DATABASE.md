...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-25-catalogue-32` release must use checksum `51f72d8bee036db17a8d988ebead98026695e6f925c510ef049f773b946b3252`. It extends the reviewed manifest from 39 to 40 rows with SKU JUW-046 as Drop 02: the crimson asymmetric draped satin maxi dress. Its public packet contains the approved 01–07 views, and inventory is initialized once without rewriting operational stock truth. The previous `2026-08-25-catalogue-31` release remains recorded by checksum `df50a7c843f0a468f343365ccdd40a326535bca3532cb287ff82adbfeb6759a7`.

The 40-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the original eighteen Drop 02 rows into Studio in the same guarded release transaction. Garment 022 is the twenty-second Drop 02 piece and is adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
