...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-22-catalogue-23` release must use checksum `dd2e42c80af4e953539aa2f6ce8c4bc5b32865facf99740a945fbd875721d394`. It extends the reviewed manifest from 30 to 31 rows with JUW-037 as Drop 02: the black cropped tee and charcoal-wash wide-leg cargo jeans set. The release uses the approved provisional price while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-22-catalogue-22` release remains recorded by checksum `23ca93e15c045f7a2821c7aa0cc50b5571eef24531ac8f628136ecd47fb9c333`.

The 31-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the thirteen new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
