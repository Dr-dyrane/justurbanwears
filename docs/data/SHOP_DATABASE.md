...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-22-catalogue-24` release must use checksum `b85f941281d5d53a7c2b7127ff0115bbe03a20cb9f657d9e2edee632a6d4e43e`. It extends the reviewed manifest from 31 to 32 rows with JUW-038 as Drop 02: the black cropped tee and dark-indigo wide-leg cargo jeans set. The release uses the established Drop 02 trousers-set provisional price while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-22-catalogue-23` release remains recorded by checksum `dd2e42c80af4e953539aa2f6ce8c4bc5b32865facf99740a945fbd875721d394`.

The 32-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the fourteen new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
