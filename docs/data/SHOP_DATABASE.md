...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-11-catalogue-08` release must use checksum `909a3209574cddaccafa89c29a1770698786772c996476dafb0ff3c975673d61`. It adds `JUW-013`, the Teal Draped Mini Set, as the fifteenth catalogue and inventory row with one available unit and six approved public media slots. Catalogue-07 remains immutable at checksum `6ea433b7d761df987ddbb9f0dd02899994277c671404931594e5af6404b112d7`. The older `db:shop:*` names remain aliases, but there is no standalone migration-only write command. Every write runs in one transaction under the shared Postgres advisory lock. The ledger treats the same namespace/revision/checksum as a no-op and rejects a reused revision with different content. Production writes additionally require a clean git worktree, so the recorded git SHA and manifest checksum identify the exact release artifact.
