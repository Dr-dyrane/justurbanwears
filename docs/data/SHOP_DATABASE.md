...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-20-catalogue-17` release must use checksum `a623f0278bad2f3aaaae363272adfd14e1f420066ece4692cbb4695c504775df`. It extends the reviewed manifest from 18 to 24 rows with JUW-025–JUW-030 as Drop 02: the black cropped tee and slim trouser set, violet beaded ruffle romper, black sweetheart fit-and-flare midi dress, black-and-ivory folded-neck column dress, indigo seamed denim mini dress, and black cropped tee with silver ruched skirt set. The release uses the approved provisional prices while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-20-catalogue-16` release remains recorded by checksum `0921c6e148cea1544ba9264614eef7bd925c7da2f9ddf826f91a96095e70c919`.

The 24-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the six new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
