...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-20-catalogue-15` release must use checksum `371d82638677221762913bc11b3f5d9392806ce60d7b8e42154e9c979981dc1d`. It extends the reviewed manifest from 18 to 22 rows with JUW-025–JUW-028 as Drop 02: the black cropped tee and slim trouser set, violet beaded ruffle romper, black sweetheart fit-and-flare midi dress, and black-and-ivory folded-neck column dress. The release uses the approved provisional prices while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy.

The 22-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the four new rows into Studio in the same guarded release transaction.
