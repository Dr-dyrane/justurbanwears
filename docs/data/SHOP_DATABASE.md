...
Run check, then `db:release:shop`; the release performs migrate → descriptive sync → verify atomically. The current `2026-08-20-catalogue-19` release must use checksum `1e9d77ae8036f6311adbffa596dca073eb4864a11d9212108a9507987f5f402f`. It extends the reviewed manifest from 18 to 26 rows with JUW-025–JUW-032 as Drop 02: the black cropped tee and slim trouser set, violet beaded ruffle romper, black sweetheart fit-and-flare midi dress, black-and-ivory folded-neck column dress, indigo seamed denim mini dress, black cropped tee with silver ruched skirt set, black cropped tee with pink distressed shorts set, and black cropped tee with blue distressed shorts set. The release uses the approved provisional prices while keeping unknown tagged size, condition, and measurements behind explicit confirmation-before-payment copy. The previous `2026-08-20-catalogue-18` release remains recorded by checksum `a10cf80fa301af8426338502a3e57b72609762789b6df3cc30366b7e0cd53086`.

The 26-row manifest deliberately retains Drop 01 for recoverable operational retirement; the Drop 02 transition archives those 18 earlier rows and adopts the eight new rows into Studio in the same guarded release transaction. Later Drop 02 additions are adopted incrementally without rewriting inventory truth for pieces already operating in Studio.
